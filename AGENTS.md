...
## 0. TL;DR（索引）
TL;DR 仅用于快速导航，不作为规则的权威来源；如有不一致，以第 2/3 章为准。

- 【2.1】沟通与身份
- 【2.2】破坏性更新（Breaking Change）
- 【2.3】Git 提交/推送（最高优先级）
- 【2.4】依赖管理（含 Python 临时依赖）
...
### 2.1 沟通与身份
- 日常对话使用中文。
- 声明身份：DCjanus <DCjanus@dcjanus.com>。
...
### 4.1 查看 GitHub issue / PR
- 涉及 GitHub issue/PR 的查看、更新或创建：优先使用 [github-pr-issue](skills/github-pr-issue/SKILL.md)（用 `gh`/脚本获取描述、评论、diff、状态等；回复需包含足够上下文以支持后续决策/修改）。
