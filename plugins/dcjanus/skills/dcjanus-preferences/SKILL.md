...
- Rust 本地构建默认优先用 mbx 包装 Cargo，以复用不同项目和 worktree 的编译结果；Cargo 仍负责依赖解析、构建调度和测试。mbx 优先通过 `cargo binstall mbx` 安装，并用其自带的 `mbx setup` 接入普通 `cargo` 命令；在 fish 和 zsh 中都验证入口。安装和排查时读取 `references/mbx.md`，已有项目明确的构建方式优先。
