...
2. 创建一个新的干净 subagent，让它看不到当前对话历史。按当前可用工具 schema 选择参数：
...
You were started as a fresh-context reviewer. Do not rely on any prior parent-agent conversation. Please inspect the final net diff yourself with git/platform CLI.
...
- 如果当前 subagent 工具不支持创建干净上下文（例如无法设置 `fork_context=false` 或 `fork_turns="none"`），停止循环并向用户说明不能保证独立审查，不要把 forked reviewer 的结果计入 clean round。
