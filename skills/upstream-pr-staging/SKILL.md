...
1. **PR 标题和正文**是正式贡献文案的预演。默认中文，但采用目标仓库的模板、叙述视角和信息密度，内容应能在去掉预审 PR 后直接交给目标仓库 reviewer。正文把目标仓库视为当前语境，不从 fork 贡献者的外部视角称其为“上游”，也不依赖 fork、bottom PR、预审阶段或内部讨论才能理解。
