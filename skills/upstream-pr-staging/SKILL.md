...
内部 draft PR 的标题和正文默认使用中文，方便用户在 fork 内部 review 和收敛；除非用户明确要求英文或目标仓库/团队规范要求英文。
...
- PR body 需要保留编号但不需要点击时，用 inline code 写成 `` `owner/repo#123` ``、`` `#123` `` 或 `` `https://github.com/owner/repo/issues/123` ``，让它作为背景文本而不是链接。
- PR body 需要引用上游 issue / PR / discussion 且希望可点击时，默认使用 Markdown 链接，并把链接目标里的 `github.com` 换成 `redirect.github.com`，不要为了低干扰改成 inline code 牺牲可点击性。这是 GitHub 官方支持的 GitHub.com 写法，不适用于 GitHub Enterprise Cloud with Data Residency。
- 如果不能确认某种写法是否会 backlink，默认不用链接，改写成普通文本背景。

示例：

```markdown
参考 [RedisShake PR 1050](https://redirect.github.com/tair-opensource/RedisShake/pull/1050)。
```
...
- 对内部 fork PR、workflow run、job、artifact、日志等确实需要点击查看的证据链接，写成 `[简短描述](URL)`。
- 链接文本要短，通常 2-6 个字即可，例如 `[失败记录](...)`、`[通过记录](...)`、`[CI](...)`；把“为什么要点这个链接、它证明什么”放在链接前后的正文里，不要把完整说明塞进 Markdown link 文本。
- 如果同一段里有多个证据链接，优先合并到一句短说明或项目符号里，避免连续堆 URL。
- 如果链接指向上游 issue / PR / discussion，仍要先遵循“背景引用”的低干扰规则；需要可点击但不想触发 backlink 时，默认用 Markdown 链接配合 `redirect.github.com`，不要写成 inline code。
- 内部 fork repo 里的 commit 证据默认只写裸露的短 SHA 文本，例如 c6bb444，不要包 inline code；GitHub 可以自动把同仓库 commit SHA 识别成可跳转引用。只有需要跨仓库跳转、消歧，或 reviewer 明确需要点击时，才写成 `[c6bb444](...)` 这类 Markdown 链接。

示例：

```markdown
提交历史按“测试有效性 -> 实现修复 -> 清理临时 CI”拆成三段，便于直接确认新增回归测试不是只覆盖最终状态。
...
3. 清理阶段
   ...
   - 保留正式回归测试和修复代码。
   - 清理提交推送后即可把必要 red/green 证据写进 PR 描述；不必先等待 cleanup 提交触发的正常 CI 全部通过。
   - 如果正常 CI 还在运行，PR 描述里简短标注对应 run 正在运行；随后继续观察 CI，必要时再更新状态。
...
9. 如果上游 maintainer 不希望 PR 历史保留 red/green/cleanup 提交，再按 reviewer 要求 squash 或整理历史。
