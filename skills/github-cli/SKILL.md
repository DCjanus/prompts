...
8. 可选正文块：仅在确有必要时再添加 `BREAKING CHANGE:`、`## Validation`、`## Risks` 或 `## Notes`；具体取舍遵循“Issue/PR 标题与正文原则”。
9. PR 正文默认先写到本地 Markdown 文件；草稿优先放 `/tmp/*.md`，不要在 shell 里拼多行字符串，也不要依赖交互式编辑。标题通常较短，可直接用 `--title` 传入。
10. 创建 PR 时优先使用 `--body-file`，例如：
```
gh pr create \
  ...
  --draft
```
11. 修改 PR 时也复用本地文件，避免手工编辑，例如：`gh pr edit <id> --title "..." --body-file /tmp/pr-body.md`。
12. 创建成功后，输出完整 PR URL。
