...
创建或更新 Issue/PR 标题与正文时，只描述当前需要提交给 reviewer/maintainer 的最终信息。PR 必须只描述目标分支当前状态到当前分支最终状态的净变化，不要按 commit 历史、开发过程、临时实验或旧正文残留来写。

- 先确认 base/head，再用 final net diff 作为正文依据：`git fetch` 后查看 `git diff --stat <base>...HEAD` 与 `git diff --name-status <base>...HEAD`；必要时再看关键文件 diff。
...
- 不要把格式化、lint、类型检查、普通单元测试、构建通过等常规卫生检查写进 PR 正文；这些通常由 CI 覆盖，信息增量低。也不要记录探索性失败、调试命令或临时注入失败，除非它是最终交付的已知风险。
- 更新已有 PR 正文时，不要在旧正文上做局部补丁；先回读当前正文，再基于 final net diff 重写完整正文并替换过时内容。
- Breaking change 也按 final net diff / 对外行为判断，不按中间 commit 机械继承；只有最终净变化确实破坏既有用法时，才在标题和正文标识。
...
3. 标题与描述遵循“Issue/PR 标题与正文原则”，内容保持简洁清晰。
4. Issue 正文默认先写到本地 Markdown 文件；草稿优先放 `/tmp/*.md`，但不要把草稿路径或其它本机路径写进正文。标题通常较短，可直接用 `--title` 传入。
...
5. 遵循“Issue/PR 标题与正文原则”。有 breaking change 时标题用 `type(scope)!: short summary`，正文加 `BREAKING CHANGE:` 说明影响和迁移方式；否则不要加 `!` 或 `BREAKING CHANGE:`。
...
8. 可选正文块：仅在确有必要时再添加。
   - `BREAKING CHANGE:`：仅当最终 PR 确实包含 breaking change 时添加。
...
9. `## Validation` 不默认引用 CI / workflow / check 证据；CI 属于合入门槛，不是正文素材。除非 pipeline 本身验证了某个 reviewer 无法从常规 CI 推断的真实场景，否则不要把 CI、格式化、lint、普通单测、构建命令写进正文。
10. PR 正文默认先写到本地 Markdown 文件；草稿优先放 `/tmp/*.md`，不要在 shell 里拼多行字符串，也不要依赖交互式编辑。标题通常较短，可直接用 `--title` 传入。
11. 创建 PR 时优先使用 `--body-file`，例如：
```
gh pr create \
  ...
  --draft
```
12. 修改 PR 时也复用本地文件，避免手工编辑，例如：`gh pr edit <id> --title "..." --body-file /tmp/pr-body.md`。
13. 创建成功后，输出完整 PR URL。

## 更新 Issue/PR 标题或描述（前置要求）
在更新 Issue 或 PR 的标题/描述之前，必须先读取当前标题/正文（即将被修改的内容），再进行修改。
更新 PR 标题或正文前，按 final net diff 重新生成整篇 PR 标题/正文，覆盖旧正文中的过时内容，并重新判断是否需要 `!` 与 `BREAKING CHANGE:`。
