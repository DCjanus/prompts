...
- `Validation` 只写 reviewer 判断最终 diff 所需的成功验证，或最终交付仍存在的明确未验证项。不要记录探索性失败、调试命令或临时注入失败，除非它是最终交付的已知风险。
...
9. `## Validation` 默认优先引用 CI / workflow / check 证据。若格式化、lint、单测或集成测试已经由 CI 覆盖，不要在正文里重复罗列本地运行过的同类命令；只补充 CI 未覆盖、但对 reviewer 判断有价值的本地或手工验证。
