...
- 先确认选型问题是否跨语言：数据库、分析、压缩或归档场景读取 `references/data-storage.md`；无序集合/多重集合摘要、增量哈希、分片摘要合并或 ECMH 选型读取 `references/multiset-hashing.md`；Protobuf 字段 presence 与兼容性判断读取 `references/protobuf.md`；Telegram Bot 消息能力选择读取 `references/telegram-bot-api.md`；语言生态库选择则读取对应语言参考文件。
- 引入或替换第三方库时优先使用偏好清单。
...
- 只有当业务对象是顺序无关、重复次数有意义的多重集合，并且确实需要增量增删或并行分片后合并摘要时，才优先评估 ECMH；一次性集合摘要优先将元素无歧义编码、排序后使用 BLAKE3。ECMH 不替代普通有序哈希、MAC、签名、加密或成员证明，具体选型与状态存储边界见 `references/multiset-hashing.md`。
...
- 无序集合、多重集合与 ECMH：`references/multiset-hashing.md`
